...
def train(train_loader, model, optimizer, epoch,save_path):
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            loss5    = structure_loss(pre_res[4], gts)

            loss_seg = loss1 + loss2 + loss3 + loss4 + loss5
            ...
            # if i == total_step or i==1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:0.4f} Loss3: {:0.4f} Loss4: {:0.4f} Loss5: {:0.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, loss1, loss2,  loss3, loss4, loss5))
                logging.info('#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:0.4f} Loss3: {:0.4f} Loss4: {:0.4f} Loss5: {:0.4f}'.
                    format( epoch, opt.epoch, i, total_step, loss1.data, loss2.data, loss3.data, loss4.data, loss5.data))
            #
        loss_all/=epoch_step
        ...
        if (epoch) % 5 == 0:
            torch.save(model.state_dict(), save_path+'TGLNet_epoch_{}.pth'.format(epoch))
            
    except KeyboardInterrupt: 
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path+'TGLNet_epoch_{}.pth'.format(epoch+1))
    ...
    with torch.no_grad():
        ...
        if epoch==1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'TGLNet_epoch_best_{}.pth'.format(epoch))
                torch.save(model.state_dict(), save_path + 'TGLNet_epoch_best.pth')
