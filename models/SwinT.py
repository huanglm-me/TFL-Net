...
class rechannel(nn.Module):
    def __init__(self, in_channel, out_channel, norm_layer=nn.BatchNorm2d):
        super(rechannel, self).__init__()
        self.conv1 = nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=1, padding=1)
        self.bn1 = norm_layer(out_channel)
        self.conv2 = nn.Conv2d(out_channel, out_channel*2, kernel_size=3, stride=1, padding=1)
        self.channel = out_channel

    def forward(self, x):
        out1 = F.relu(self.bn1(self.conv1(x)), inplace=True)
        out2 = self.conv2(out1)
        p, t = out2[:, :self.channel, :, :], out2[:, self.channel:, :, :]
        return F.relu(p * out1 + t, inplace=True)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=384, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.rc4 = rechannel(1024, 256)
        self.rc3 = rechannel(512, 256)
        self.rc2 = rechannel(256, 256)
        self.rc1 = rechannel(128, 256)
    ...
    def forward_features(self, x):
        ...
        f1 =  self.rc1(layer_features[0])
        f2 =  self.rc2(layer_features[1])
        f3 =  self.rc3(layer_features[2])
        f4 =  self.rc4(layer_features[3])
